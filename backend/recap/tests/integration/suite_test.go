...
func runTests(m *testing.M) int {
	...
	cancel()
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "set up integration environment: %v\n", err)
	...
	teardownCancel()
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "tear down integration environment: %v\n", err)
...
func postgresConfig(
	...
	if err != nil {
		return config.PostgreSQLConfig{}, fmt.Errorf(
	...
	if err != nil {
		return config.PostgreSQLConfig{}, fmt.Errorf(
			"parse postgres connection string: %w",
			err,
		)
	}
...
func (e *environment) resetDatabase(t *testing.T) {
	t.Helper()
